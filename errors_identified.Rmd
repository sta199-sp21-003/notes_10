...
- Error #1: missing quotation in date
- Error #2: author in YAML header misspelled
- Error #3: YAML header missing a -
- Error #4: `nycflights13` not included in `load-packages` code chunk
- Error #5: `load-packages` below code chunk using `dplyr`
- Error #6: `load-packages` code chunk has option `eval = FALSE`
- Error #7: duplicate code chunk name `late-flights`
- Error #8: incorrect code chunk formatting of `cinnabon`: `{r}{cinnabon}`
- Error #9: `count()` in `jfk-delay` should be `n()`
- Error #10: `flights` in `jfk-delay` should be `origin`
- Error #11: missing `na.rm` argument in `jfk-delay`
- Error #12: variables in `jfk-delay` `summarize()` not named
- Error #13: evaluate `jfk-delay`
- Error #14: stray `+` in `plot-evening-flights`
- Error #15: `jfk_monthly` misspelled in `plot-evening-flights`
- Error #16: missing `mapping = aes(...)` in `plot-evening-flights` 
- Error #17: didn't close out `rdu-ontime` code chunk with backticks
- Error #18: `rdu-ontime` has `=` not `==`
- Error #19: missing `%>%` in `rdu-ontime`
- Error #20: incorrect code chunk formatting in `barplot-by-dest`
- Error #21: `%in%` not `in` in `barplot-by-dest`
- Error #22: should be `geom_violin` in `boxplots-by-dest`
- Error #23: argument of `geom_violin()` is `draw_quantiles` not `quantiles`
- Error #24: `distance / (air_time / 60)` in `boxplots-by-dest`
- Error #25: `delays-by-month` code chunk not properly formatted: two backticks
- Error #26: missing final `)` in `delays-by-month`
- Error #27: unexpected $\mu$ character at end of document
